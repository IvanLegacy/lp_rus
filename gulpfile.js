...
gulp.task('dev:webpack', function (callback) {
	const options = require('./webpack')(isDevelopment);
...
gulp.task('dev:js', function () {
	return gulp
		.src([path.assets.js + '*.js', '!' + path.assets.js + 'main.js'])
		.pipe(strip())
		.pipe(gulp.dest(path.build.js))
		.pipe(livereload());
});
...
gulp.task('dev:video', function () {
	return gulp.src(path.assets.video).pipe(gulp.dest(path.build.video));
});

// assembly fonts
gulp.task('dev:fonts', function () {
	return gulp.src(path.assets.fonts).pipe(gulp.dest(path.build.fonts));
});
...
gulp.task(
	...
	gulp.parallel(
		'dev:scss',
		gulp.series('dev:webpack', 'dev:js'),
		'dev:img',
		'dev:video',
		'dev:fonts',
		gulp.series('dev:svg', 'dev:html')
	)
);
...
gulp.task('copy', gulp.series('bower', 'copy:js', 'copy:css'));
...
gulp.task(
	'init',
	gulp.series('copy', 'dev', gulp.parallel('watch', 'server'))
);
